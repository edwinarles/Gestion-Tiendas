...
# Asegurar la columna store_id en la base de datos de usuarios (por si ya existía la tabla)
db_session = UsersSessionLocal()
try:
    db_session.execute(text("ALTER TABLE users ADD COLUMN store_id VARCHAR;"))
    db_session.commit()
except Exception:
    db_session.rollback()
...
# 2. Listar todas las tiendas (Filtrable por vendedor)
@app.get("/api/stores", response_model=List[schemas.StoreResponse])
def list_stores(role: str = None, store_id: str = None, db: Session = Depends(get_db)):
    query = db.query(models.Store)
    if role == "vendedor" and store_id:
...
# --- ENDPOINTS PARA GESTIÓN DE USUARIOS (SÓLO ACCESIBLES POR EL PROPIETARIO) ---

@app.get("/api/users", response_model=List[schemas.UserListResponse])
def get_users(db: Session = Depends(get_users_db)):
    return db.query(User).order_by(User.id.asc()).all()

@app.post("/api/users", response_model=schemas.UserListResponse, status_code=status.HTTP_201_CREATED)
def create_user(user_in: schemas.UserCreateRequest, db: Session = Depends(get_users_db)):
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"El usuario '{username_clean}' ya existe"
        )
    ...
    new_user = User(
        ...
        role=user_in.role,
        store_id=user_in.store_id if user_in.role == "vendedor" else None
    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def delete_user(user_id: int, db: Session = Depends(get_users_db)):
    ...
    # Prevenir eliminar el propietario por defecto
    if user.username == "propietario":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No se puede eliminar el usuario administrador 'propietario'"
        )
